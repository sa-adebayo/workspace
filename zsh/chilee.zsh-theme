...
prompt_pure_preprompt_render() {
	...
	if [[ -n $prompt_pure_vcs_info[branch] ]]; then
		git_info=("%F{blue}${vcs_info_msg_0_%%/.} %F{$git_color}"'${prompt_pure_vcs_info[branch]}${prompt_pure_git_dirty}%f')
		info=`echo $git_info`
		preprompt_parts+=("${info}")
...
prompt_pure_async_vcs_info() {
	...
	zstyle ':vcs_info:git*' actionformats "%b(%a)%u%c" '%R'
